...
new #[Layout('layouts.auth')] class extends Component
{
    ...
    {
        return $this->view()->title(__('titles.password.request'));
    }
};
?>
...
<div>
    @if (!$sent)
        <div class="mb-8">
            <h1 id="forgot-heading" class="font-heading text-3xl text-heading mb-1.5">{{ __('auth.forgot_heading') }}</h1>
            <p class="text-sm text-subtle">{{ __('auth.forgot_subtitle') }}</p>
        </div>
    ...
    @else
        <div class="mb-8">
            <h1 class="font-heading text-3xl text-heading mb-1.5">{{ __('auth.forgot_sent_heading') }}</h1>
            <p class="text-sm text-subtle">{{ __('auth.forgot_sent_subtitle') }}</p>
        </div>
        <div class="rounded-

        [6px] bg-dark/5 border border-dark/10 px-5 py-4 text-sm text-subtle leading-relaxed">
